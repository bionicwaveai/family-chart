...
- **Whole-tree overview**: `examples/big-tree.html` reads the same `/api/tree` data and shows the whole tree fit-to-screen. The library draws the tree around one "main" person (their ancestors + all descendants + spouses), so the page picks the main that maximizes coverage (`bestMainId`) and enables `setShowSiblingsOfMain` to render as many people as possible at once; a "Showing N of M" indicator and a "Show everyone" button make this explicit. Click a person to focus their branch, or search by name. Reachable from the icon rail (the tree icon below the menu button, wired in `examples/sidebar.js`).
